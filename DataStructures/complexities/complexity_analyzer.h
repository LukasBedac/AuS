...
namespace ds::utils
{
    ...
    void ComplexityAnalyzer<Structure>::runReplications(Structure structurePrototype)
    {
        ...
        //definuj struktury, do ktorych ulozime namerane udaje
        ...
        { 
            ...
            Structure structureTest(structurePrototype);
            ...
            for (size_t i = 0; i < getStepCount(); i++)
            {
                growToSize(structureTest, sizesvector[i]);
                
                beforeOperation_(structureTest);
                auto timeStart = std::chrono::high_resolution_clock::now();
                executeOperation(structureTest);
                auto timeFinish = std::chrono::high_resolution_clock::now();
                afterOperation_(structureTest);
                
                duration_t duration = std::chrono::duration_cast<duration_t>(timeFinish - timeStart);                
                durationsVector.push_back(duration);
            }

            //vloz vektor do matice

            durationMatrix.push_back(std::move(durationsVector));
        }

        //uloz udaje
        saveToCsvFile(sizesvector, durationMatrix);
    }
